...
public class DataProcessorTask implements Runnable{
	...
	public void run() {
		...
		if (crc != calc_crc) {
			...
			default: {
				if (bArray[2] == DataListener.isCarData || bArray[2] == DataListener.isRebootCarData) {
					// car data
	...
	public void ProcessTopoData(byte[] topoArray) {
		...
		String sessionId = String.format("%02x", topoArray[DataListener.session_index]);
		...
		if (statusId != 0) {
			...
			String firmwareStr = buildHex(topoArray, DataListener.firmware_index, 2);
			String unicastStr = buildHex(topoArray, DataListener.unicast_index, 2);
